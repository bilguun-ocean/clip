...
if os.path.exists(embeddings_file):
    ...
    english_embeddings = [eval(emb) for emb in df['eng_embedding']]  # Convert string back to list
...
def get_optimal_batch_size():
    ...
    batch_size = 512  # Start high for L100
    while batch_size > 1:
        try:
...
for epoch in range(3):  # Example: 3 epochs
    ...
    with tqdm(total=len(mongolian_sentences), desc=f'Epoch {epoch + 1}', ncols=100) as pbar:
        for i in range(0, len(mongolian_sentences), BATCH_SIZE):
            ...
            batch_english_embeddings = torch.tensor(english_embeddings[i:i+BATCH_SIZE]).to(device)

            with torch.cuda.amp.autocast():  # Mixed precision training
                batch_student_embeddings = multilingual_model.forward(batch_mongolian_sentences, tokenizer).to(device)
                loss = torch.nn.functional.cosine_similarity(batch_student_embeddings, batch_english_embeddings, dim=-1).mean()
