...
# Load your fine-tuned M-CLIP Model and Tokenizer
model_name = 'fine_tuned_multilingual_clip'  # Update this with your model path or name
device = "cuda" if torch.cuda.is_available() else "cpu"
model = pt_multilingual_clip.MultilingualCLIP.from_pretrained(model_name).to(device)
tokenizer = transformers.AutoTokenizer.from_pretrained('xlm-roberta-large')  # Or use the tokenizer from your model
...
for image_id in tqdm(dataset['image_id'].unique(), desc="Processing Images"):
    ...
    # Compute embeddings using your fine-tuned M-CLIP model
    with torch.no_grad():
